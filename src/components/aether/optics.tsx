...
import { Camera, ExternalLink, Plane, Satellite, Shield } from "lucide-react";
import { Badge } from "@/components/ui/badge";
import { kmLabel } from "@/lib/uap/format";
import { getCameraFrame, getWatchContext } from "@/lib/uap/live";
import type { CameraHit, Contact, LiveTrackOptics } from "@/lib/uap/types";
    ...
    queryFn: () =>
      getWatchContext({
        data: {
          ...
          durationSec: contact!.durationSec,
          ...
          <ul className="mt-2 space-y-2">
            {data.safety.map((s) => (
              <li key={s.label} className="flex items-start justify-between gap-3">
                <div>
                  <p className={s.level === "elevated" ? "text-sm text-alert" : "text-sm text-fg"}>{s.label}</p>
                  <p className="text-xs leading-relaxed text-muted">{s.note}</p>
                </div>
                <span className="font-mono text-[10px] uppercase tracking-[0.12em] text-muted">{s.level}</span>
              </li>
            ))}
          </ul>
        </div>
...
export function LiveOpticsStrip({
  tracks,
  pending,
  onOpen,
}: {
  tracks: LiveTrackOptics[];
  pending: boolean;
    ...
    <div className="border-b border-border bg-surface">
      <div className="flex items-center justify-between gap-2 px-3 pt-2 sm:px-4">
        <p className="font-mono text-[10px] uppercase tracking-[0.14em] text-muted">
          {pending ? "Acquiring live optics…" : "Live track optics"}
        </p>
        <p className="font-mono text-[10px] uppercase tracking-[0.12em] text-subtle">
          {tracks.length} residual{tracks.length === 1 ? "" : "s"} · {tiles.length} frames
        </p>
      </div>
      <div className="flex gap-2 overflow-x-auto px-3 py-2 sm:px-4">
        {tiles.length === 0 && (
          <p className="py-4 text-xs text-muted">
            {pending
              ? "Pulling public cameras with a line of sight on live residuals."
              : "No public camera in optical range of the current residuals. Geostationary frames still apply on inspect."}
          </p>
        )}
        {tiles.map(({ cam, track }) => (
          <button
